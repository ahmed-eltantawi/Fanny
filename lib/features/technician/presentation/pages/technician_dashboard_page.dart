...
class _TechnicianDashboardPageState extends State<TechnicianDashboardPage>
  ...
  void _load() {
    context
        .read<RequestsCubit>()
        .loadRequests(const GetRequestsParams(role: 'technician'));
  }
            ...
            SliverAppBar(
              ...
              bottom: TabBar(
                ...
                labelStyle: const TextStyle(
                    ...
                    fontSize: 13),
                tabs: [
                  Tab(text: l.availableRequests),
                  Tab(text: l.myActiveJobs)
                ],
              ),
            ),
          ],
...
class _TechnicianHeader extends StatelessWidget {
  ...
  Widget build(BuildContext context) {
    ...
    return FutureBuilder<List<RequestEntity>>(
      future: user == null
          ? Future.value(const <RequestEntity>[])
          : sl<FirestoreAppDataService>()
              .getTechnicianAssignedRequests(user!.id),
      builder: (context, snapshot) {
        final jobs = snapshot.data ?? const <RequestEntity>[];
        final completed =
            jobs.where((job) => job.status == RequestStatus.completed).length;
                    ...
                    child: Row(
                      children: [
                        ...
                        _StatPill(
                            label: l.earnings,
                            value:
                                '${(completed * 350).toStringAsFixed(0)} ج.م'),
                      ],
                    ),
                  ),
                ],
                  ...
                  itemBuilder: (context, i) => FadeInUp(
                    ...
                    child: Padding(
                      ...
                      child: _AvailableRequestCard(
                          request: requests[i], l: l, technicianUser: user),
                    ),
                  ),
                );
              }
...
class _AvailableRequestCard extends StatelessWidget {
  ...
  const _AvailableRequestCard(
      {required this.request, required this.l, this.technicianUser});
              ...
              children: [
                ...
                BlocBuilder<OffersCubit, OffersState>(
                  builder: (_, state) => AppButton(
                    ...
                    onTap: technicianUser == null
                        ? null
                        : () {
                            context
                                .read<OffersCubit>()
                                .submitOffer(SubmitOfferParams(
                                  ...
                                  technicianRating:
                                      technicianUser!.rating ?? 4.5,
                                  technicianCompletedJobs:
                                      technicianUser!.completedJobs ?? 0,
                                  technicianSpecialty:
                                      technicianUser!.specialty ?? '',
...
class _MyJobsTab extends StatelessWidget {
  ...
  Widget build(BuildContext context) {
    ...
    return FutureBuilder<List<RequestEntity>>(
      future:
          sl<FirestoreAppDataService>().getTechnicianAssignedRequests(user!.id),
      builder: (context, snapshot) {
        if (snapshot.connectionState == ConnectionState.waiting) {
          return const ShimmerRequestList(count: 2);
        }
        final myJobs = snapshot.data ?? const <RequestEntity>[];
        if (myJobs.isEmpty) {
          return Center(
              child: FadeIn(
                  child: Column(mainAxisSize: MainAxisSize.min, children: [
